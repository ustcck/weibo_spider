...
from weibo_spider.items import InformationItem, TweetsItem
...
class Spider(CrawlSpider):
    ...
    def start_requests(self):
        while True:
            ...
            url_tweets = "http://weibo.cn/%s?page=868" % ID
    ...
    def parse0(self, response):
        ...
        if text0:
            ...
            informationItems["Weibo_ID"] = response.meta["ID"]
    ...
    def parse2(self, response):
        ...
        for tweet in tweets:
            ...
            content_a = tweet.xpath('div/span[@class="ctt"]/a/text()').extract_first()  # 微博内容里面的超链接
            if not content_a:
                continue
            if '消防知识' not in content_a.encode("utf-8"):
                continue
            ...
            tweetsItems["WeiboID"] = response.meta["ID"]
            tweetsItems["ID"] = response.meta["ID"] + "-" + id
            tweetsItems["Topic"] = '消防知识'
            if content:
